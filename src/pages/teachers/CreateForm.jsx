...
function CreateForm() {
  ...
  const onSubmit = async function (data) {
    ...
    const validation = validateTeacher(data, false);
    ...
    const { confirmPassword, ...submitData } = validation.value;

    try {
      const response = await axios.post(`${HOSTNAME}/a/teacher`, submitData);
      if (response.status === 200) {
        redirect("/teachers", { state: { message: "เพิ่มครูเรียบร้อยแล้ว" } });
      }
    } catch (error) {
      console.error(error);
      if (error.response && error.response.data) {
        ...
        const errorMessages = {
          tchCode:
            serverErrors.tchCode === "duplicate"
              ? "รหัสครูนี้มีอยู่ในระบบแล้ว"
              : "",
          email:
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        value={
                          department.find((dept) => dept.deptId === field.value)
                            ? {
                                value: field.value,
                                label: department.find(
                                  (dept) => dept.deptId === field.value
                                ).deptName,
                              }
                            : null
                        }
