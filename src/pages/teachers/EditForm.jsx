...
import { validateTeacher } from "../../validator.js";
...
function EditForm() {
      ...
      .then(([deptResponse, teacherResponse]) => {
        ...
        if (teacherData.deptId) {
          const currentDept = departments.find(
            (dept) => dept.deptId === teacherData.deptId
          );
          if (currentDept) {
            const deptOption = {
              value: currentDept.deptId,
              label: currentDept.deptName,
            };
  ...
  const onSubmit = async function (data) {
    // Validate using Yup schema for edit mode
    const validation = validateTeacher(data, true);
    ...
    // Remove confirmPassword and empty password fields before submitting
    const { confirmPassword, password, ...submitData } = validation.value;
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    รหัสผ่านใหม่{" "}
                    <span className="text-text-color-alt text-xs">
