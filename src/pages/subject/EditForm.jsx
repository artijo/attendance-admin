...
import { validateSubject } from "../../validator.js";
...
function EditForm() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([typesRes, teachersRes, subjectRes, subjectsRes]) => {
        ...
        const currentTeacher = teacherOptions.find(
          (t) => t.value === subjectRes.data.tchId
        );
  ...
  const onSubmit = async function (data) {
    // Add teacher ID to data
    const submitData = {
      ...data,
      tchId: selectedTeacher?.value || null,
    };

    // Validate using Yup
    const validation = validateSubject(submitData);

    if (!validation.success) {
      // Display first error as alert
    ...
    try {
      ...
      if (error.response && error.response.data) {
        ...
        if (serverError.subCode === "duplicate") {
          setValidationErrors({ subCode: "รหัสวิชานี้มีอยู่ในระบบแล้ว" });
          setInputError({
            title: "เกิดข้อผิดพลาด",
            description: "รหัสวิชานี้มีอยู่ในระบบแล้ว",
          });
        } else {
              ...
              >
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.subCode ? "border-red-500" : ""
                    }`}
                    {...register("subCode")}
                    onChange={(e) => {
                      register("subCode").onChange(e);
                      if (validationErrors.subCode) {
                        setValidationErrors({
                          ...validationErrors,
                          subCode: "",
                        });
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    step="0.5"
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.subCredit ? "border-red-500" : ""
                    }`}
                    {...register("subCredit", { valueAsNumber: true })}
                    onChange={(e) => {
                      register("subCredit").onChange(e);
                      if (validationErrors.subCredit) {
                        setValidationErrors({
                          ...validationErrors,
                          subCredit: "",
                        });
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    id="subNameThai"
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.subNameThai ? "border-red-500" : ""
                    }`}
                    {...register("subNameThai")}
                    onChange={(e) => {
                      register("subNameThai").onChange(e);
                      if (validationErrors.subNameThai) {
                        setValidationErrors({
                          ...validationErrors,
                          subNameThai: "",
                        });
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    id="subNameEng"
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.subNameEng ? "border-red-500" : ""
                    }`}
                    {...register("subNameEng")}
                    onChange={(e) => {
                      register("subNameEng").onChange(e);
                      if (validationErrors.subNameEng) {
                        setValidationErrors({
                          ...validationErrors,
                          subNameEng: "",
                        });
                ...
                <div className="space-y-2">
                  ...
                  <select
                    id="subTypeId"
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.subTypeId ? "border-red-500" : ""
                    }`}
                    {...register("subTypeId")}
                    onChange={(e) => {
                      register("subTypeId").onChange(e);
                      if (validationErrors.subTypeId) {
                        setValidationErrors({
                          ...validationErrors,
                          subTypeId: "",
                        });
