...
function Activities() {
    ...
    return (
        <div className="min-h-screen">
            <div className="mb-6">
                <h1 className="text-2xl md:text-3xl font-bold text-primary font-heading">กิจกรรม</h1>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
                {!isLoading && (
                    <div className="mb-3 sm:mb-0 bg-white rounded-lg px-4 py-2 border border-line shadow-sm">
                        <span className="text-text-color-alt font-body">จำนวนกิจกรรมทั้งหมด:</span>
                        <span className="ml-2 font-medium text-primary text-lg font-heading">{totalActivities} กิจกรรม</span>
                        <div className="mt-1 flex items-center gap-4 text-xs">
                            <div className="flex items-center gap-1.5">
                                <span className="w-3 h-3 rounded-full bg-blue-500"></span>
                                <span>กิจกรรมต่อเนื่อง: {continuousActivities?.length || 0}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                                <span className="w-3 h-3 rounded-full bg-green-500"></span>
                                <span>กิจกรรมไม่ต่อเนื่อง: {nonContinuousActivities?.length || 0}</span>
                            </div>
                        </div>
                    </div>
                )}
