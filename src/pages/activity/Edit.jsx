...
function EditActivity() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([activityRes, actTypesRes, teachersRes, classroomsRes]) => {
        ...
        const selectedTeachers = isAllTeachers
          ? []
        ...
        const selectedClassrooms = activity.classroom
          ? activity.classroom.map((c) => ({
              value: c.classroom.classId,
              label: `ม.${c.classroom.classLevel}/${c.classroom.classRoom} - ${
                c.classroom.classroomType?.classTypeNameThai || ""
              }`,
            }))
          : [];
  ...
  const onSubmit = (data) => {
    ...
    axios
      .put(HOSTNAME + "/a/activity/" + id, activityData)
      .then(() => {
      ...
      .catch((error) => {
        ...
        setInputError({
          title: "เกิดข้อผิดพลาด",
          description: "ไม่สามารถแก้ไขกิจกรรมได้ กรุณาลองใหม่อีกครั้ง",
        });
      });
  };
