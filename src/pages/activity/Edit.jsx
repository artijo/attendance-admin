...
function EditActivity() {
    ...
    useEffect(() => {
        ...
        Promise.all([
        ...
        ]).then(([activityRes, actTypesRes, teachersRes, classroomsRes]) => {
            ...
            // Format all activity types for dropdown
            const activityTypeOptions = actTypesRes.data.map(type => ({
            ...
            // Format classrooms
            const classroomOptions = classroomsRes.data.map(classroom => ({
                value: classroom.classId,
                label: `ม.${classroom.classLevel}/${classroom.classRoom} - ${classroom.classroomType.classTypeNameThai} (ปีการศึกษา ${classroom.term.academicYear + 543})`
            }));
            setClassrooms(classroomOptions);

            // Determine join limit type
            const joinLimitType = activity.joinLimitNumber ? 'number' : 'classroom';

            // Check if all teachers are selected for this activity
            const isAllTeachers = activity.teacherAll;
            setAllTeachersSelected(isAllTeachers);

            // Format selected teachers
            const selectedTeachers = isAllTeachers ? [] : activity.teacher.map(t => ({
                value: t.teacher.tchId,
                label: `${t.teacher.fName} ${t.teacher.lName}`
            }));

            // Format selected classrooms
            const selectedClassrooms = activity.classroom ? activity.classroom.map(c => ({
                value: c.classroom.classId,
                label: `ม.${c.classroom.classLevel}/${c.classroom.classRoom} - ${c.classroom.classroomType?.classTypeNameThai || ""}`
            })) : [];
            ...
            reset({
                ...
                joinLimitType: joinLimitType,
                joinLimitNumber: activity.joinLimitNumber || "",
    ...
    return (
        <div className="min-h-screen">
            <div className="mb-6">
                <h1 className="text-2xl md:text-3xl font-bold text-primary font-heading">แก้ไขข้อมูลกิจกรรม</h1>
                                ...
                                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                    ...
                                    {joinLimit && (
                                        <div className="sm:col-span-2 bg-gray-50 p-4 rounded-lg border border-gray-100">
                                            <p className="text-sm font-medium text-text-color mb-3 font-body">ประเภทการจำกัดการเข้าร่วม</p>
                                            <div className="flex gap-6 mb-4">
                                                <label className="inline-flex items-center">
                                                    ...
                                                    />
                                                    <span className="ml-2 text-text-color font-body">จำกัดตามห้องเรียน</span>
                                                </label>
                                                <label className="inline-flex items-center">
                                                    <input
                                                        type="radio"
                                                        ...
                                                        className="form-radio h-4 w-4 text-primary"
                                                    />
                                                    <span className="ml-2 text-text-color font-body">จำกัดตามจำนวนผู้เข้าร่วม</span>
                                                </label>
                                            </div>
                                            ...
                                            {joinLimitType === 'classroom' && (
                                                <div className="space-y-2">
                                                    <label className="text-sm font-medium text-text-color font-body">
                                                        ห้องเรียนที่สามารถเข้าร่วมได้ <span className="text-red-500">*</span>
                                                    </label>
                                                    <Controller
                                                        name="classrooms"
                                                        control={control}
