...
import { validateClassroom } from "../../validator.js";
...
function EditClassroom() {
  ...
  const onSubmit = async function (data) {
    // Validate using Yup
    const validation = validateClassroom(data);
    ...
    try {
      const response = await axios.put(`${HOSTNAME}/a/classroom`, {
        ...validation.value,
        classId: id,
      });
      ...
      if (error.response && error.response.data) {
        ...
        setError(serverError.message || "เกิดข้อผิดพลาดในการแก้ไขห้องเรียน");
      } else {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (classroom.classTeacher?.length > 0) {
          ...
          setValue(
            "teacherIds",
            classroom.classTeacher.map((t) => t.tchId)
          );
          ...
          setTeacherOptions((prevOptions) => {
            ...
            const newTeachers = teacherData.filter(
              (t) => !existingIds.includes(t.value)
            );
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                value={
                  classroomType?.find(
                    (ct) => ct.classTypeId === watch("classTypeId")
                  )
                    ? {
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                value={academicterms
                  ...
                  .find(
                    (option) =>
                      academicterms?.find((t) => t.termId === watch("termId"))
                        ?.termId === option.value
                  )}
                onChange={(selectedOption) => {
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onChange={(sel) =>
                  setValue("teacherIds", sel ? sel.map((o) => o.value) : [])
                }
                isClearable
                isMulti
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                value={studentOptions?.find(
                  (option) => option.value === watch("leaderId")
                )}
                onChange={(selectedOption) =>
                  setValue(
                    "leaderId",
