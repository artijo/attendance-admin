...
import { validateClassroomType } from "../../../validator.js";
...
const ClassroomTypeManage = () => {
  ...
  const isProtectedType = (type) => {
    return (
      type.classTypeNameEng === "Unspecified" ||
      type.classTypeNameThai === "ไม่ระบุ"
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate using Yup
    const validation = validateClassroomType(formData);
    ...
    try {
      if (editingId) {
        await axios.put(
          `${HOSTNAME}/a/classroom/type/${editingId}`,
          validation.value
        );
      } else {
        await axios.post(`${HOSTNAME}/a/classroom/type`, validation.value);
      }
      setSuccessful({
        title: `${editingId ? "แก้ไข" : "บันทึก"}สำเร็จ`,
        description: `${editingId ? "แก้ไข" : "เพิ่ม"}ประเภทห้องเรียนสำเร็จ`,
      });
      ...
      if (error.response && error.response.data) {
        ...
        if (
          serverError.classTypeNameThai === "duplicate" ||
          serverError.classTypeNameEng === "duplicate"
        ) {
          setValidationErrors({
            general: "ชื่อประเภทห้องเรียนนี้มีอยู่ในระบบแล้ว",
          });
        } else {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `${HOSTNAME}/a/classroom/type/${classroomTypeToDelete.classTypeId}`
      );
      ...
      setSuccessful({
        title: "ลบสำเร็จ",
        description: "ลบประเภทห้องเรียนนั้นสำเร็จ",
      });
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-8 text-center border border-line">
          ...
          <h2 className="text-xl font-semibold text-text-color mb-2 font-heading">
            ไม่พบข้อมูลประเภทห้องเรียน
          </h2>
          <p className="text-text-color-alt font-body">
            กรุณาเพิ่มประเภทห้องเรียนใหม่
          </p>
        </div>
      ) : (
                ...
                {classroomTypes.map((type) => (
                  <tr
                    ...
                    className={`hover:bg-gray-50 transition-colors duration-150 ${
                      isProtectedType(type) ? "bg-gray-50/50" : ""
                    }`}
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="inline-flex overflow-hidden rounded-md border border-line bg-white shadow-sm">
                        <button
                          ...
                          onClick={() =>
                            !isProtectedType(type) && handleEdit(type)
                          }
                          title={
                            isProtectedType(type)
                              ? "ไม่สามารถแก้ไขประเภทนี้ได้"
                              : "แก้ไขข้อมูล"
                          }
                        ...
                        <button
                          ...
                          onClick={() =>
                            !isProtectedType(type) && handleDelete(type)
                          }
                          title={
                            isProtectedType(type)
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md border border-line">
            <div className="text-center mb-5">
              ...
              <p className="text-text-color-alt font-body">
                คุณต้องการลบประเภทห้องเรียน "
                {classroomTypeToDelete?.classTypeNameThai}" ใช่หรือไม่?
              </p>
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setFormData({
                        ...formData,
                        classTypeNameThai: e.target.value,
                      });
                      if (validationErrors.classTypeNameThai) {
                        setValidationErrors({
                          ...validationErrors,
                          classTypeNameThai: "",
                        });
                    ...
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.classTypeNameThai
                  ...
                  <p className="text-xs text-text-color-alt mt-1 font-body">
                    ตัวอย่าง: ห้องเรียนปกติ, ห้องเรียนพิเศษ
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setFormData({
                        ...formData,
                        classTypeNameEng: e.target.value,
                      });
                      if (validationErrors.classTypeNameEng) {
                        setValidationErrors({
                          ...validationErrors,
                          classTypeNameEng: "",
                        });
                    ...
                    className={`w-full rounded-lg border-gray-300 py-2.5 px-3 shadow-sm focus:border-primary focus:ring-primary font-body text-text-color ${
                      validationErrors.classTypeNameEng
                        ? "border-red-500 focus:border-red-500 focus:ring-red-500"
                        : ""
                    }`}
                  ...
                  <p className="text-xs text-text-color-alt mt-1 font-body">
                    ตัวอย่าง: Regular Class, Special Class
                  </p>
                </div>
              </div>
