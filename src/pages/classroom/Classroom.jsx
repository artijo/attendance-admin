...
function Classroom() {
  ...
  const academicYears = useMemo(() => {
    ...
    const years = [
      ...new Set(allClassrooms.map((classroom) => classroom.term.academicYear)),
    ];
  ...
  const filteredClassrooms = useMemo(() => {
    ...
    if (selectedGrade !== "all") {
      filtered = filtered.filter(
        (classroom) => parseInt(classroom.classLevel) == selectedGrade
      );
    }
    ...
    if (selectedYear !== "all") {
      filtered = filtered.filter(
        (classroom) => classroom.term.academicYear == selectedYear
      );
    }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        {filteredClassrooms && (
          <div className="mb-3 sm:mb-0 bg-white rounded-lg px-4 py-2 border border-line shadow-sm">
            ...
            {filteredClassrooms.length !== totalClassrooms &&
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl shadow-md p-8 text-center border border-line">
          ...
          <p className="text-text-color-alt font-body">
            กรุณาเพิ่มห้องเรียนหรือเปลี่ยนตัวกรอง
          </p>
          {(selectedGrade !== "all" || selectedYear !== "all") && (
            <button
              onClick={resetFilters}
