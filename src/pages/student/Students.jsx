    ...
    if (search !== "") {
      filteredStudents = filteredStudents.filter(
        (student) =>
          ...
          student.fName?.concat(" ", student.lName).toLowerCase().includes(search.toLowerCase()) ||
          student.lName?.concat(" ", student.fName).toLowerCase().includes(search.toLowerCase())
      );
    }
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="w-full sm:w-1/2">
              ...
              >
                ...
                {Object.entries(classroomsByLevel).sort((a, b) => a[0] - b[0]).map(([level, rooms]) => (
                  <optgroup key={level} label={`มัธยมศึกษาปีที่ ${level}`}>
                    {/* Option for all rooms in this level */}
                    <option value={`${level}`}>{`ม.${level} (ทุกห้อง)`}</option>
                    
                    {/* Individual rooms */}
                    {rooms.sort((a, b) => a.classRoom - b.classRoom).map((classroom) => (
                      <option key={classroom.classId} value={`${classroom.classLevel}-${classroom.classRoom}`}>
            ...
            <div className="relative w-full sm:w-1/2">
              <label htmlFor="Search" className="block text-sm font-medium text-text-color font-body mb-2 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
