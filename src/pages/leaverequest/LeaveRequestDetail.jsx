...
function LeaveRequestDetail() {
      ...
      <div className="min-h-screen">
        <div className="bg-white rounded-xl shadow-md p-8 text-center border border-line">
          ...
          <p className="text-text-color-alt font-body">
            ไม่พบข้อมูลการลาที่ต้องการหรือข้อมูลถูกลบไปแล้ว
          </p>
          <div className="mt-6">
            <Link
              to="/leavereq"
  ...
  const classroom =
    leaveRequest.studingTime[0]?.studingTime?.timetable?.classroom;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-line">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-lg font-semibold text-text-color font-heading">
              ข้อมูลนักเรียน
            </h2>
          </div>
          <div className="space-y-3">
            <div className="flex flex-col">
              <span className="text-sm text-text-color-alt font-body">
                รหัสนักเรียน
              </span>
              <span className="font-medium text-text-color">
                {leaveRequest.student.stdId}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-sm text-text-color-alt font-body">
            ...
            {classroom && (
              <div className="flex flex-col">
                ...
                <span className="font-medium text-text-color">
                  มัธยมศึกษาปีที่ {classroom.classLevel} ห้อง{" "}
                  {classroom.classRoom}
                </span>
              </div>
            )}
            <div className="flex flex-col">
              ...
              <span className="font-medium text-text-color">
                {leaveRequest.student.tel
                  ? formatPhoneNumber(leaveRequest.student.tel)
                  : "-"}
              </span>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-line">
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="font-medium text-text-color whitespace-pre-wrap">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-line">
          ...
          {leaveRequest.LeaveFile ? (
              ...
              <div className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-text-color truncate">
                    {leaveRequest.LeaveFile.filename}
                  </p>
                  <p className="text-xs text-text-color-alt">
                    อัพโหลดเมื่อ: {formatThaiDateTime(leaveRequest.createdAt)}
                  </p>
                </div>
                <a
            ...
            <div className="text-center py-6">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-12 w-12 mx-auto text-text-color-alt mb-2"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
      ...
      <div className="mt-6">
        <h2 className="text-xl font-semibold text-text-color font-heading mb-4">
          รายละเอียดคาบที่ลา
        </h2>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-text-color-alt uppercase tracking-wider bg-gray-50 border-b border-line">
                <tr>
                  ...
                  <th className="px-6 py-3">ดำเนินการมื่อ</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div>
                          {formatTimeThai(
                            studyTime.studingTime.timetable.timeStart,
                          )}{" "}
                          -{" "}
                          {formatTimeThai(
                            studyTime.studingTime.timetable.timeEnd,
                          )}
                        </div>
                        <div className="text-sm text-text-color-alt">
                          วัน
                          {getDayOfWeekThai(
                            studyTime.studingTime.timetable.dayOfWeek,
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${getStatusBadgeClass(studyTime.leaveStatus)}`}
                        >
                          {getStatusText(studyTime.leaveStatus)}
                        </span>
                        <span className="text-sm text-text-color-alt block mt-1">
                          {studyTime.rejectedNote &&
                            studyTime.leaveStatus === "REJECTED" && (
                              <> หมายเหตุ: {studyTime.rejectedNote}</>
                            )}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        {studyTime.teacherApprove ? (
                          <div className="text-text-color">
                            {studyTime.teacherApprove.fName}{" "}
                            {studyTime.teacherApprove.lName}
                          </div>
                        ) : (
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-blue-700">
                การอนุมัติรายการลาต้องทำโดยครูผู้สอนเท่านั้น
                ผู้ดูแลระบบไม่สามารถอนุมัติหรือปฏิเสธคำขอลาได้
              </p>
            </div>
          </div>
        </div>
