...
function AddHoliday({ holidayList, setHolidayList, termStart, termEnd, callApiAddHoliday, prevStep, isSubmitting}) {
    ...
    const handleCheckboxChange = (index) => {
        ...
        const isCheck = selectedHolidays.findIndex((holidayIndex) => holidayIndex === index); // find index จะ return เป็น -1 ถ้าไม่เจอ
        // console.log(isCheck);
        if (isCheck < 0) {
    ...
    const handleAddHoliday = (e) => {
        e.preventDefault();
        const holidayRangeList = daybetween(holiday.startDate, holiday.endDate).map((date) => {
            return {
    ...
    return (
        <div className="mt-4">
            {holidayList.length > 0 && (
                <div className="my-4 w-full flex justify-end">
                    <button
                        ...
                        className={`px-3 py-1 rounded ${selectedHolidays.length > 0
                ...
                <form onSubmit={handleAddHoliday}>
                    <table className="w-full text-sm text-left rtl:text-right">
                        ...
                        <tbody>
                            ...
                            {sliceHolidayList.length > 0 && sliceHolidayList.map((holiday, index) => (
                                <tr key={`${holiday.holidayname}-${index}`} className="border-b border-gray-200">
                ...
                <div className="py-4">
                    <div className="flex items-center justify-between">
                        <p className="text-sm text-text-color-alt">
                            แสดง <span className="font-medium text-text-color">{sliceHolidayList.length}</span> จาก <span className="font-medium text-text-color">{holidayList.length}</span> รายการ
                        </p>
            ...
            <div className="flex justify-end gap-2 mt-3">
                ...
                <button
                    onClick={() => callApiAddHoliday()}
                    disabled={holidayList.length === 0}
