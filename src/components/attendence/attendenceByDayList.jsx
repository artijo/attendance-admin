...
export const AttendenceByDayList = ({ termId, classroomId }) => {
  ...
  const sliceDayList = dayList.slice(
              ...
              {sliceDayList.length > 0 ? (
                sliceDayList.map((day, index) => {
                  ...
                  const dayOfWeek = formatDayOfWeeks(
                    DateTime.fromISO(`${day}T17:00:00`).setZone(TIME_ZONE)
                      .weekday,
                  );
                  ...
                  return (
                    <tr
                      key={index}
                      className={`${isWeekend ? "bg-red-50" : "bg-white"} hover:bg-gray-50 transition-colors duration-150`}
                    >
                      <td className="px-6 py-4 font-medium text-text-color">
                        {formattedDate}
                      </td>
                      <td
                        className={`px-6 py-4 ${isWeekend ? "text-red-600 font-medium" : "text-text-color"}`}
                      >
                        {dayOfWeek}
                      </td>
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-10 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="font-medium text-text-color mb-1">
                        ไม่พบข้อมูลวันที่มีเรียน
                      </h3>
                      <p className="text-sm text-text-color-alt">
                        ไม่มีวันเรียนที่ถูกกำหนดในเทอมนี้
                      </p>
                    </div>
                  </td>
                </tr>
