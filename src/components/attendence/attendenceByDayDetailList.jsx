...
export const AttendanceByDayDetailList = ({ studentList }) => {
    ...
    const setupTotalStatus = () => {
        ...
        const periods = studentList[0].attendance.length;
        const totals = [];
        ...
        studentList.forEach((student) => {
            student.attendance.forEach((attendance, periodIndex) => {
                if (attendance.attStatus !== null) {
                    ...
                    if (totals[periodIndex].hasOwnProperty(status)) {
                        totals[periodIndex][status]++;
                    }
                }
            });
        });
    ...
    const handlePdfComponent = () => {
        if (studentList.length > 0 && periodTotals.length > 0 && classroomInfo) {
            return <ByDay studentList={studentList} totalStatus={periodTotals} date={date} classroomInfo={classroomInfo} />;
        }
        return null;
    };
    ...
    return (
        <div>
            {studentList.length > 0 && periodTotals.length > 0 && classroomInfo && (
                        ...
                        <tfoot>
                            <tr className="bg-gray-50">
                                ...
                                {periodTotals.map((total, index) => (
                                    <td key={`present-${index}`} className="px-6 py-3 border-r border-gray-200 text-green-600 font-medium text-center">
                                        {total.present || 0}
                                    </td>
                                ))}
                            </tr>
                            <tr className="bg-gray-50">
                                ...
                                {periodTotals.map((total, index) => (
                                    <td key={`late-${index}`} className="px-6 py-3 border-r border-gray-200 text-orange-500 font-medium text-center">
                                        {total.late || 0}
                                    </td>
                                ))}
                            </tr>
                            <tr className="bg-gray-50">
                                ...
                                {periodTotals.map((total, index) => (
                                    <td key={`absent-${index}`} className="px-6 py-3 border-r border-gray-200 text-red-600 font-medium text-center">
                                        {total.absent || 0}
                                    </td>
                                ))}
                            </tr>
                            <tr className="bg-gray-50">
                                ...
                                {periodTotals.map((total, index) => (
                                    <td key={`leave-${index}`} className="px-6 py-3 border-r border-gray-200 text-purple-600 font-medium text-center">
                                        {total.leave || 0}
                                    </td>
                                ))}
                            </tr>
                            <tr className="bg-gray-50">
                                ...
                                {periodTotals.map((total, index) => (
                                    <td key={`activity-${index}`} className="px-6 py-3 border-r border-gray-200 text-blue-600 font-medium text-center">
                                        {total.activity || 0}
                                    </td>
                                ))}
                            </tr>
