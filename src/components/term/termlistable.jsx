  ...
  const handleOnDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${HOSTNAME}/a/academicterms/${termToDelete.termId}`,
      );
      if (response.status === 200) {
        await fetchData();
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center p-8">
          <svg
            ...
            className="h-16 w-16 text-text-color-alt mb-4"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
          ...
          <p className="text-text-color-alt font-body">
            กรุณาเพิ่มข้อมูลปีการศึกษาและเทอม
          </p>
        </div>
      ) : (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-line">
                <th className="px-4 py-3.5 text-left text-xs font-medium text-text-color-alt tracking-wider font-heading">
                  ปีการศึกษา
                </th>
                <th className="px-4 py-3.5 text-left text-xs font-medium text-text-color-alt tracking-wider font-heading">
                  เทอม
                ...
                <th className="px-4 py-3.5 text-center text-xs font-medium text-text-color-alt tracking-wider font-heading">
                  จัดการ
                </th>
              </tr>
            </thead>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md border border-line">
            <div className="text-center mb-5">
              ...
              <p className="text-text-color-alt font-body">
                คุณต้องการลบข้อมูลปีการศึกษา {termToDelete.academicYear + 543}{" "}
                เทอม {termToDelete.semester} ใช่หรือไม่?
              </p>
            </div>
